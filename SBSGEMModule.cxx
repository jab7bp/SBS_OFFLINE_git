...
#include "/w/halla-scshelf2102/sbs/jboyd/SBS_OFFLINE/SBS-offline/GEM_lookups.h"
...
int crosstalk_cnt = 0;
int CrosstalkStripCount = 0;
int CrosstalkRemovalCount = 0;
int fXtalk_ADCsum_size = 0;

ULong64_t crosstalk_zero_set_cnt = 0;
ULong64_t crosstalk_corrected_adc_cnt = 0;

ULong64_t fXtalkCnt=0;
ULong64_t fXtalkRemovalCnt=0;
ULong64_t fXtalkStripCnt=0;
ULong64_t fXtalkZeroSetCnt=0;
ULong64_t fXtalkADCsumNonzeroCnt=0;
ULong64_t fXtalkADCsumNonzeroCntAfter=0;
ULong64_t fXtalkADCsumNonzeroZSCnt=0;
ULong64_t fXtalkADCsumNonzeroZSCntAfter=0;
ULong64_t fXtalk_Nstrips_hit_pos = 0;
ULong64_t fXtalk_Nstrips_keep = 0;
ULong64_t fXtalk_Nstrips_ontrack = 0;
ULong64_t fXtalkHardCutoff = 0;
ULong64_t fXtalkADCsumNonzeroLoopCnt = 0;
ULong64_t xtalkloopdummy = 0;
ULong64_t nonzeroZSdummy = 0;
ULong64_t nonzeroZSdummyAfter = 0;

ULong64_t nonzerodummy = 0;
ULong64_t nonzerodummyAfter = 0;
ULong64_t ontrackdummy = 0;

bool build_ADCmax = false;
bool build_ADCsum = true;
bool build_event_viewer = false;

Double_t get_xtalk_threshold(int runnum, int mod_num, int UorV, int apv_select){

  TString filename("/w/halla-scshelf2102/sbs/jboyd/SBS_REPLAY/SBS-replay/DB/gemXtalk/db_xtalk_bb_gem_run13770.dat");

  std::ifstream xtalk_ratio_file( filename.Data() );

  TString currentline;
  TString current_val("");

  while( currentline.ReadLine( xtalk_ratio_file ) ){
    if( currentline.BeginsWith( Form("m%i_U", mod_num ) ) ){
      TObjArray *tokens = currentline.Tokenize( " " );

      current_val = ( (TObjString*) (*tokens)[apv_select + 1] )->GetString();
      break;
    }
  }
  return atof( current_val.Data() );
}
...
Int_t SBSGEMModule::ReadDatabase( const TDatime& date ){
  ...
  int crosstalk_flag = fCrosstalk ? 1 : 0;
  ...
  const DBRequest request[] = {
    ...
    { "crosstalk_analysis", &crosstalk_flag, kUInt, 0, 1, 1}, //(optional, search): Turn crosstalk corrections ON(1)/OFF(0)
  ...
  fXtalkEvent_U.resize( nstripsmax );
  fChanXtalkNumer_U.resize( nstripsmax );
  fChanXtalkDenom_U.resize( nstripsmax );
  fStripXtalkDenom_U.resize( nstripsmax );
  fStripXtalkNumer_U.resize( nstripsmax );

  fXtalkEvent_V.resize( nstripsmax );
  fChanXtalkNumer_V.resize( nstripsmax );
  fChanXtalkDenom_V.resize( nstripsmax );
  fStripXtalkDenom_V.resize( nstripsmax );
  fStripXtalkNumer_V.resize( nstripsmax );
...
///Reading in Crosstalk DB if the crosstalk flag is set:
...
Int_t SBSGEMModule::DefineVariables( EMode mode ) {
  ...
  VarDef varstrip[] = {
    ...
    { "xtalkCnt", "Total count of steps into the if( fCrosstalk ) statement", kInt, 0, &fXtalkCnt},
    { "xtalkRemovalCnt", "Total number of strips removed in the crosstalk correction process", kInt, 0, &fXtalkRemovalCnt},
    { "xtalkStripCnt", "Total number of strips looked at in the crosstalk analysis process", kInt, 0, &fXtalkStripCnt},
    { "xtalkCorrectedADCcnt", "Total number of strips that get corrected (not set to 0) in Crosstalk Analysis", kInt, 0, &fXtalkCorrectedADCcnt},
    { "xtalkZeroSetCnt", "Total number of strips that get set to 0 in crosstalk analysis", kInt, 0, &fXtalkZeroSetCnt},
    { "fXtalkADCsumNonzeroCnt", "Total number of non-zero strips in ADCsums before crosstalk analysis", kInt, 0, &fXtalkADCsumNonzeroCnt},
    { "fXtalkADCsumNonzeroCntAfter", "Total number of non-zero strips in ADCsum after crosstalk analysis", kInt, 0, &fXtalkADCsumNonzeroCntAfter},
    { "fXtalkADCsumNonzeroZSCnt", "Total number of non-zero strips in ADCsums before Zero suppression", kInt, 0, &fXtalkADCsumNonzeroZSCnt},
    { "fXtalkADCsumNonzeroZSCntAfter", "Total number of non-zero strips in ADCsums after Zero suppression", kInt, 0, &fXtalkADCsumNonzeroZSCntAfter},
    { "fXtalkNstripsfired", "Number of strips fired", kUInt, 0, &fXtalk_Nstrips_hit_pos },
    { "fXtalkNstripskeep", "Number of strips passing basic timing cuts", kUInt, 0, &fXtalk_Nstrips_keep },
    { "fXtalkNstripsOnTrack", " Number of strips on track", kUInt, 0, &fXtalk_Nstrips_ontrack}, 
    { "fXtalkHardCutoff", "Is Crosstalk Hard cut-off enabled?", kUInt, 0, &fXtalkHardCutoff},
    { "fXtalkADCsumNonzeroLoopCnt", "Count for nonzero ADCsum from inside Crosstalk loop before stuff happens", kInt, 0, &fXtalkADCsumNonzeroLoopCnt},
    { nullptr },
  };
...
Int_t   SBSGEMModule::Decode( const THaEvData& evdata ){
  ...
  for(size_t adc = 0; adc < fADCsums.size(); adc++ ){
    if( fADCsums[adc] > 0 ){
      nonzeroZSdummy++;
      fXtalkADCsumNonzeroZSCnt = nonzeroZSdummy;
    }
  }
  ...
  int nonzero_ontrack = 0;
  for(size_t s = 0; s < fStripOnTrack.size(); s++){
    if(fStripOnTrack[s] == 1){
      nonzero_ontrack++;
      fXtalk_Nstrips_ontrack += 1;
    }
  }
  ...
  for(size_t adc = 0; adc < fADCsums.size(); adc++ ){
    if( fADCsums[adc] > 0 ){
      nonzerodummy++;
      fXtalkADCsumNonzeroCnt = nonzerodummy;
      fXtalkADCsumNonzeroZSCntAfter = nonzerodummy;
    }
  }

  // fIsDecoded = true;
  int ADCsum_size_before = fADCsums.size();
  int this_zero_cnt;
  int this_corr_cnt;

  if( fCrosstalk ){
    // cout << "Beginning crosstalk analysis. " << endl;
    //Following segment was written by John Boyd
    //Begins the assessment and correction of crosstalk in the GMn & nTPE data.

    int ADCmax_cut = 500;
    int ADCsum_cut = fXtalk_ADCcut_val;



    fXtalk_ADCsum_size += fADCsums.size();

    TString module_num_name = fName;
    module_num_name.ReplaceAll("m", "");  

    // cout << "Module name: " << module_num_name << endl;

    // int number_of_modules = lookup_nModules_total(runnum);
    int module_num = std::atoi(module_num_name);
    int gem_type = lookup_GEM_type(fXtalk_runnum, module_num);
    // std::cout << "runnum: " << fXtalk_runnum << "gemtype: " << gem_type << ", nAPVs_U: " << fNAPVs_U << ", nAPVs_V: " << fNAPVs_V << std::endl;
    int nAPVs_U = fNAPVs_U;
    int nAPVs_V = fNAPVs_V;

    bool CrosstalkHardCutoff = false;
    if( CrosstalkHardCutoff ){
      fXtalkHardCutoff = 1;
    }
    else{ fXtalkHardCutoff = 0;}
    double xtalk_ratio_threshold = 4;

    // cout << "Creating output files. " << endl;
    if( false ){
      ofstream crosstalk_output;
      crosstalk_output.open("/u/home/jboyd/xtalk/crosstalk_output.txt");
    }
...
//############################################################################
  ...
  if( build_ADCsum ){
      Double32_t APV_chan_ADCsum_U[nAPVs_U][128];
      Double32_t APV_chan_ADCsum_U_corrected[nAPVs_U][128];
      Double32_t APV_chan_ADCsum_V[nAPVs_V][128];
      Double32_t APV_chan_ADCsum_V_corrected[nAPVs_V][128];
      Int_t APV_chan_isampmax_ADCsum_U[nAPVs_U][128];
      Int_t APV_chan_isampmax_ADCsum_V[nAPVs_V][128];

      int chan_U;
      int chan_V;
 
      // cout << "Fetching ADCsum values...." << endl;
      for(size_t i = 0; i < fADCsums.size(); i++){
        chan_U = -1;
        chan_V = -1;
    //-------------U APVs
        if(fStripIsU[i] == 1){
          int apv_U = strip_to_APV(int(fStrip[i]));
          chan_U = GEM_strip_to_channel(gem_type, int(fStrip[i])%128);
          // std::cout << "apv u: " << apv_U << ", chan u: " << chan_U << std::endl;
          APV_chan_ADCsum_U[apv_U][chan_U] = fADCsums[i];
          APV_chan_ADCsum_U_corrected[apv_U][chan_U] = fADCsums[i];

          APV_chan_isampmax_ADCsum_U[apv_U][chan_U] = fMaxSamp[i];
        }

    //-------------V APVs
        if(fStripIsV[i] == 1){
          int apv_V = strip_to_APV(int(fStrip[i]));
          chan_V = GEM_strip_to_channel(gem_type, int(fStrip[i])%128);
          APV_chan_ADCsum_V[apv_V][chan_V] = fADCsums[i];
          APV_chan_ADCsum_V_corrected[apv_V][chan_V] = fADCsums[i];

          APV_chan_isampmax_ADCsum_V[apv_V][chan_V] = fMaxSamp[i];
        }
...
//New VERSION:::
    //The ADCsums array is now sorted into the APV_chan_ADCsum arrays by apv and chan
    //Need to loop through the ADCsums vector again and now correspond i strips in fADCsums to chan in apv/chan array
      this_zero_cnt = 0;
      this_corr_cnt = 0;
      ...
      for( size_t i = 0; i < fADCsums.size(); i++ ){
        ...
        if( fADCsums[i] > 0 ){
          xtalkloopdummy++;
          fXtalkADCsumNonzeroLoopCnt = xtalkloopdummy;
        }

        if( fStripIsU[i] == 1 ){

          CrosstalkStripCount++;
          fXtalkStripCnt = CrosstalkStripCount;

          int apv_U = strip_to_APV(int(fStrip[i]));
          chan_U = GEM_strip_to_channel(gem_type, int(fStrip[i])%128);

          if( fXtalk_ratios_U[apv_U] < 0 ){
            // std::cout << "Xtalk ratio value less than 0 for U APV: " << apv_U << std::endl;
            continue;
          }
          else{
            double this_ratio = 0.0;
            double correction = 0.0;
            double corrected_ADC = 0.0;
            int large_strip;
            int small_strip;
            int large_chan;
            int small_chan;

            if( (chan_U > 0) && (APV_chan_ADCsum_U[apv_U][chan_U] > 0) && (APV_chan_ADCsum_U[apv_U][chan_U-1] > 0) ){
              if( ( APV_chan_ADCsum_U[apv_U][chan_U] > APV_chan_ADCsum_U[apv_U][chan_U - 1] ) && (APV_chan_ADCsum_U[apv_U][chan_U] > ADCsum_cut) && ( APV_chan_isampmax_ADCsum_U[apv_U][chan_U] == APV_chan_isampmax_ADCsum_U[apv_U][chan_U - 1] ) ){
                this_ratio = APV_chan_ADCsum_U[apv_U][chan_U]/APV_chan_ADCsum_U[apv_U][chan_U - 1];
                large_chan = chan_U;
                small_chan = chan_U - 1;

                large_strip = GEM_channel_to_strip(gem_type, large_chan);
                small_strip = GEM_channel_to_strip(gem_type, small_chan);
              }

              if( (APV_chan_ADCsum_U[apv_U][chan_U - 1] > APV_chan_ADCsum_U[apv_U][chan_U]) && (APV_chan_ADCsum_U[apv_U][chan_U - 1] > ADCsum_cut) && ( APV_chan_isampmax_ADCsum_U[apv_U][chan_U] == APV_chan_isampmax_ADCsum_U[apv_U][chan_U - 1] ) ){
                this_ratio = APV_chan_ADCsum_U[apv_U][chan_U - 1]/APV_chan_ADCsum_U[apv_U][chan_U];
                large_chan = chan_U - 1;
                small_chan = chan_U;
              ...
              if( (this_ratio >= xtalk_ratio_threshold) ){

                CrosstalkRemovalCount++;
                fXtalkRemovalCnt = CrosstalkRemovalCount;

                if( CrosstalkHardCutoff ){
                  fADCsums[i] = 0;
                  crosstalk_zero_set_cnt++;
                  fXtalkZeroSetCnt = crosstalk_zero_set_cnt;
                  fXtalkCorrectedADCcnt = 0;
                }
                if( !CrosstalkHardCutoff ){

                  correction = APV_chan_ADCsum_U[apv_U][large_chan]/fXtalk_ratios_U[apv_U];
                  corrected_ADC = APV_chan_ADCsum_U[apv_U][small_chan] - correction;
                  ...
                  if( corrected_ADC > 0 ){
                    fADCsums[i] = corrected_ADC;
                    this_corr_cnt++;
                    crosstalk_corrected_adc_cnt++;
                    fXtalkCorrectedADCcnt = crosstalk_corrected_adc_cnt;
                  }
                }
              }
            }
  ...
  //START OF V-STRIPS      
        if( fStripIsV[i] == 1 ){
          ...
          if( fXtalk_ratios_V[apv_V] < 0 ){
            ...
            if( (chan_V > 0) && (APV_chan_ADCsum_V[apv_V][chan_V] > 0) && (APV_chan_ADCsum_V[apv_V][chan_V-1] > 0) ){
              if( ( APV_chan_ADCsum_V[apv_V][chan_V] > APV_chan_ADCsum_V[apv_V][chan_V - 1] ) && (APV_chan_ADCsum_V[apv_V][chan_V] > ADCsum_cut) && ( APV_chan_isampmax_ADCsum_V[apv_V][chan_V] == APV_chan_isampmax_ADCsum_V[apv_V][chan_V - 1] ) ){
                this_ratio = APV_chan_ADCsum_V[apv_V][chan_V]/APV_chan_ADCsum_V[apv_V][chan_V - 1];
                large_chan = chan_V;
                small_chan = chan_V - 1;
              ...
              if( (APV_chan_ADCsum_V[apv_V][chan_V - 1] > APV_chan_ADCsum_V[apv_V][chan_V]) && (APV_chan_ADCsum_V[apv_V][chan_V - 1] > ADCsum_cut) && ( APV_chan_isampmax_ADCsum_V[apv_V][chan_V] == APV_chan_isampmax_ADCsum_V[apv_V][chan_V - 1] ) ){
                this_ratio = APV_chan_ADCsum_V[apv_V][chan_V - 1]/APV_chan_ADCsum_V[apv_V][chan_V];
                large_chan = chan_V - 1;
                small_chan = chan_V;
              ...
              if( (this_ratio >= xtalk_ratio_threshold) ){
                ...
                if( !CrosstalkHardCutoff ){
                  ...
                  correction = APV_chan_ADCsum_V[apv_V][large_chan]/fXtalk_ratios_V[apv_V];
                  corrected_ADC = APV_chan_ADCsum_V[apv_V][small_chan] - correction;
  ...
  //END OF V-STRIPS
      ...
      for(int apv_clr = 0; apv_clr < nAPVs_U; apv_clr++){
        // strips_ontrack.clear();
        for(int i = 0; i < 128; i++){
          APV_chan_ADCsum_U[apv_clr][i] = 0;
          // APV_chan_ADCsum_U_corrected[apv_clr][i] = 0;
        }
      }
      ...
      for(int apv_clr = 0; apv_clr < nAPVs_V; apv_clr++){
        // strips_ontrack.clear();
        for(int i = 0; i < 128; i++){
          APV_chan_ADCsum_V[apv_clr][i] = 0;
          // APV_chan_ADCsum_V_corrected[apv_clr][i] = 0;

        }
      }

      // crosstalk_output << "legnth of fADCsums: " << fADCsums.size() << endl;
      crosstalk_cnt++;
      fXtalkCnt = crosstalk_cnt;
...
//-------------------------  
  ...
  for(size_t adc = 0; adc < fADCsums.size(); adc++ ){
    if( fADCsums[adc] > 0 ){
      nonzerodummyAfter++;
      fXtalkADCsumNonzeroCntAfter = nonzerodummyAfter;
    }
  }


  int ADCsum_size_after = fADCsums.size();
  // if( fCrosstalk ){
  //   if( crosstalk_cnt%500 == 0 ){
  //     std::cout << "Crosstalk count: " << crosstalk_zero_set_cnt + crosstalk_corrected_adc_cnt << std::endl;
  //     // std::cout << "Removal count: " << CrosstalkRemovalCount << std::endl;
  //     // std::cout << "Strip Count: " << CrosstalkStripCount << std::endl;
  //     // std::cout << "--------------------------------------------" << std::endl;
  //     // std::cout << "Currently looking at layer: " << fLayer << std::endl;
  //     // std::cout << "--------------------------------------------" << std::endl;  
  //     // std::cout << "fADCsum length: " << fADCsums.size() << std::endl;
  //   }
...
Double_t SBSGEMModule::CorrCoeff( int nsamples, const std::vector<double> &Usamples, const std::vector<double> &Vsamples ){
  ...
  if ( (int)Usamples.size() < nsamples || (int)Vsamples.size() < nsamples ){
