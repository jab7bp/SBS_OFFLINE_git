...
// #include "/work/halla/sbs/jboyd/xtalk/include/GEM_lookups.h"
// #include "/w/halla-scshelf2102/sbs/jboyd/SBS_OFFLINE/SBS-offline/GEM_lookups.h"

using namespace std;
Long64_t fXtalkNtracks_found = 0;
...
void SBSGEMTrackerBase::CompleteInitialization(){
  ...
  bool xtalk_already_loaded = false;

  std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
  std::cout << "Crosstalk setting in DB: " << fModules[0]->fCrosstalk << std::endl;
  std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
  ...
  for( int imod=0; imod<(int)fModules.size(); imod++ ){
    ...
    fModules[imod]->fXtalk_ratios_U.resize(fModules[imod]->fNAPVs_U);
    fModules[imod]->fXtalk_ratios_V.resize(fModules[imod]->fNAPVs_V);
  ...
  if( fModules[0]->fCrosstalk ){
    if( !fXtalkfilename.empty() ){

      LoadCrosstalkRatios( fXtalkfilename.c_str() );
    }
  }
  
}
void SBSGEMTrackerBase::LoadCrosstalkRatios( const char *fname ){
  
  TString fXtalkfilename = fname;

  TString prefix = std::getenv("DB_DIR");
  prefix += "/";

  std::cout << "[SBSGEMTrackerBase::LoadCrosstalkRatios]: fname = " << fname << std::endl;
  

  if( gSystem->AccessPathName( fname ) ){ //
    
    std::cout << "[SBSGEMTrackerBase::LoadCrosstalkRatios]: could not find " << fname << " in working directory, looking in " << prefix << std::endl;

    fXtalkfilename.Prepend(prefix);
    
  }

  std::ifstream fXtalkfile( fXtalkfilename.Data() );

  if( !fXtalkfile.good() ){
    fXtalkfile.close();

    std::cout << "Warning: could not find Crosstalk Ratios file " << fname << " in working directory or in " << prefix << ", Crosstalk Ratios not loaded" << std::endl;

    return;
  } else {
    std::cout << "JBs offline directory." << endl;
    std::cout << "Found Crosstalk Ratios file " << fXtalkfilename << endl;
  }

  int num_modules = (int)fModules.size();
  //Read through DB file to get maximum number of modules and apvs for setup.

  std::cout << "Num modules: " << num_modules << std::endl;

  vector<Int_t> apv_U_cnt(num_modules, 0);
  vector<Int_t> apv_V_cnt(num_modules, 0);

  TObjArray *file_tokens = fXtalkfilename.Tokenize("_");
  int num_tokens = file_tokens->GetEntries();

  TString run_token;
  TString ADCcut_token;
  run_token.Form( ( (TObjString*) (*file_tokens)[num_tokens - 1] )->GetString() );
  run_token.ReplaceAll("run", "");
  run_token.ReplaceAll(".dat", "");

  double current_U_val = 0;
  double current_V_val = 0;

  fXtalkfile.ifstream::clear();
  fXtalkfile.ifstream::seekg(0, fXtalkfile.beg);

  TString currentline;
  // TString current_val("");

//Pull xtalk ratio values:
  while( currentline.ReadLine( fXtalkfile ) ){
    if( currentline.BeginsWith("0") || currentline.BeginsWith("1") || currentline.BeginsWith("2") || currentline.BeginsWith("3") || currentline.BeginsWith("4") || currentline.BeginsWith("5") || currentline.BeginsWith("6") || currentline.BeginsWith("7") || currentline.BeginsWith("8") || currentline.BeginsWith("9")){
      TObjArray *first_line_tokens = currentline.Tokenize("_");
      int num_first_line_tokens = first_line_tokens->GetEntries();
      ADCcut_token.Form( ((TObjString*) (*first_line_tokens)[num_first_line_tokens - 1] )->GetString() );
      ADCcut_token.ReplaceAll("ADCcut", "");
      for(int imod = 0; imod<(int)fModules.size(); imod++){
        fModules[imod]->fXtalk_runnum = stoi(run_token.Data());
        fModules[imod]->fXtalk_ADCcut_val = stoi(ADCcut_token.Data());
      }
      
      std::cout << "Values from DB file: Runnum: " << fModules[0]->fXtalk_runnum << ", Num modules: " << num_modules <<", ADCcut: " << fModules[0]->fXtalk_ADCcut_val << std::endl;
    }
    for(int mod = 0; mod < num_modules; mod++){

      if( currentline.BeginsWith( Form("m%i_U", mod)) ){

        TObjArray *line_tokens = currentline.Tokenize( " " );
        if( line_tokens->GetLast() < (fModules[mod]->fNAPVs_U) ){
          std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
          std::cout << "Missing some values in the Xtalk DB file. Module and APV info: " << std::endl;
          std::cout << "U Module " << mod << ", nAPVs_U expected: " << fModules[mod]->fNAPVs_U << ", nAPVs_U in DB file: " << line_tokens->GetLast() << std::endl;
          std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
        }
        for(int apv_U = 0; apv_U < (fModules[mod]->fNAPVs_U); apv_U++){
          current_U_val = std::stof(( ( (TObjString*) (*line_tokens)[apv_U + 1] )->GetString() ).Data());
          fModules[mod]->fXtalk_ratios_U[apv_U] = current_U_val;
          current_U_val = 0;
          apv_U_cnt[mod]++;
        }
      }

      if( currentline.BeginsWith( Form("m%i_V", mod)) ){
        // std::cout << "Found line for V strip on Module " << mod << std::endl;

        TObjArray *line_tokens = currentline.Tokenize( " " );
        if( line_tokens->GetLast() < (fModules[mod]->fNAPVs_V) ){
          std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
          std::cout << "Missing some values in the Xtalk DB file. Module and APV info: " << std::endl;
          std::cout << "V Module " << mod << ", nAPVs_V expected: " << fModules[mod]->fNAPVs_U << ", nAPVs_V in DB file: " << line_tokens->GetLast() << std::endl;
          std::cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << std::endl;
        }
        // std::cout << "Size of current line: " << line_tokens->GetLast() << std::endl;
        // std::cout << "NAPVs_V: " << (fModules[mod]->fNAPVs_V) << std::endl;        
        for(int apv_V = 0; apv_V < (fModules[mod]->fNAPVs_V); apv_V++){
          current_V_val = std::stof(( ( (TObjString*) (*line_tokens)[apv_V + 1] )->GetString() ).Data());
          fModules[mod]->fXtalk_ratios_V[apv_V] = current_V_val;
          current_V_val = 0;
          apv_V_cnt[mod]++;
          // std::cout << "mod: " << mod << ", apv_V_cnt: " << apv_V_cnt[mod] << endl;         
        }
      }
    }
  }
  // std::cout << ", Number of modules detected from Crosstalk DB file: " << num_modules << std::endl;
  std::cout << "Finsished reading through Crosstalk DB file. Here is the summary:" << std::endl;
  for(int i = 0; i < num_modules; i++){
    std::cout << "Module: " << i << ", nAPVs_U: " << apv_U_cnt[i] << ", nAPVs_V: " << apv_V_cnt[i] << endl;
  }

  if(true){
    ofstream xtalk_test_output;
    std::cout << "Writing xtalk ratio thresholds from DB file to output file. " << std::endl;
    xtalk_test_output.open("/v/lustre19/expphy/volatile/halla/sbs/jboyd/xtalk_test_output.txt");
    xtalk_test_output << "Output file for Xtalk ratios read in from DB file...." << endl;
    xtalk_test_output << "Run: " << fModules[0]->fXtalk_runnum << ", ADCcut = " << fModules[0]->fXtalk_ADCcut_val << endl;

    for(int mod = 0; mod < num_modules; mod++){

      xtalk_test_output << Form("m%i_U ", mod);

      for(int apv_U = 0; apv_U < apv_U_cnt[mod]; apv_U++){
        xtalk_test_output << fModules[mod]->fXtalk_ratios_U[apv_U] << " ";
      }
      xtalk_test_output << endl;
    }
    xtalk_test_output << endl;

    for(int mod = 0; mod < num_modules; mod++){  

      xtalk_test_output << Form("m%i_V ", mod);

      for(int apv_V = 0; apv_V < apv_V_cnt[mod]; apv_V++){
        xtalk_test_output << fModules[mod]->fXtalk_ratios_V[apv_V] << " ";
      }
      xtalk_test_output << endl;
    }
    xtalk_test_output << endl;
...
void SBSGEMTrackerBase::LoadPedestals( const char *fname ){
  ...
  prefix += "/";
  std::cout << "***********************************************" << std::endl;
  std::cout << "Running this from JB's SBS_OFFLINE directory. " << std::endl;
  std::cout << "***********************************************" << std::endl;
...
void SBSGEMTrackerBase::InitEfficiencyHistos(const char *dname){
  ...
  if( fMakeEfficiencyPlots && !fEfficiencyInitialized ){
    ...
    h_xtalk_cnt = new TH1D("h_xtalk_cnt", "h_xtalk_cnt", 1, 0.5, 1.5);
    h_xtalk_removal_cnt = new TH1D("h_xtalk_removal_cnt", "h_xtalk_removal_cnt", 1, 0.5, 1.5);
    h_xtalk_strip_cnt = new TH1D("h_xtalk_strip_cnt", "h_xtalk_strip_cnt", 1, 0.5, 1.5);
    h_xtalk_ADCcut = new TH1D("h_xtalk_ADCcut", "h_xtalk_ADCcut", 1, 0.5, 1.5);  
    h_xtalk_corrected_adc_cnt = new TH1D("h_xtalk_corrected_adc_cnt", "h_xtalk_corrected_adc_cnt", 1, 0.5, 1.5);
    h_xtalk_zero_set_cnt = new TH1D("h_xtalk_zero_set_cnt", "h_xtalk_zero_set_cnt", 1, 0.5, 1.5);

    h_xtalk_ADCsum_nonzero_cnt = new TH1D("h_xtalk_ADCsum_nonzero_cnt", "h_xtalk_ADCsum_nonzero_cnt", 1, 0.5, 1.5);
    h_xtalk_ADCsum_nonzero_cnt_after = new TH1D("h_xtalk_ADCsum_nonzero_cnt_after", "h_xtalk_ADCsum_nonzero_cnt_after", 1, 0.5, 1.5);
    h_xtalk_ADCsum_nonzero_cnt_ALL = new TH1D("h_xtalk_ADCsum_nonzero_cnt_ALL", "h_xtalk_ADCsum_nonzero_cnt_ALL", 1, 0.5, 1.5);

    h_xtalk_ADCsum_nonzero_ZS_cnt = new TH1D("h_xtalk_ADCsum_nonzero_ZS_cnt", "h_xtalk_ADCsum_nonzero_ZS_cnt", 1, 0.5, 1.5);
    h_xtalk_ADCsum_nonzero_ZS_cnt_after = new TH1D("h_xtalk_ADCsum_nonzero_ZS_cnt_after", "h_xtalk_ADCsum_nonzero_ZS_cnt_after", 1, 0.5, 1.5);
    h_xtalk_ADCsum_nonzero_ZS_cnt_ALL = new TH1D("h_xtalk_ADCsum_nonzero_ZS_cnt_ALL", "h_xtalk_ADCsum_nonzero_ZS_cnt_ALL", 2, 0.5, 2.5);

    h_xtalk_hardcutoff = new TH1D("h_xtalk_hardcutoff", "h_xtalk_hardcutoff", 1, 0.5, 1.5);

    h_xtalk_nstripsfired = new TH1D("h_xtalk_nstripsfired", "h_xtalk_nstripsfired", 1, 0.5, 1.5);
    h_xtalk_ntracks_found = new TH1D("h_xtalk_ntracks_found", "h_xtalk_ntracks_found", 1, 0.5, 1.5);
    h_xtalk_nstrips_keep = new TH1D("h_xtalk_nstrips_keep", "h_xtalk_nstrips_keep", 1, 0.5, 1.5);
    h_xtalk_nstrips_ontrack = new TH1D("h_xtalk_nstrips_ontrack", "h_xtalk_nstrips_ontrack", 1, 0.5, 1.5);

    h_xtalk_total_stripcounter = new TH1D("h_xtalk_total_stripcounter", "xtalk_total_stripcounter", 1, 0.5, 1.5);
    h_xtalk_XtalkADCsumNonzeroLoopCnt = new TH1D("h_xtalk_XtalkADCsumNonzeroLoopCnt", "h_xtalk_XtalkADCsumNonzeroLoopCnt", 1, 0.5, 1.5);
